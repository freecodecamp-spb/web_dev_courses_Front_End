...
`Merge pull request`.
О правилах названия комитов и веток ниже.

## Как назвать комит
Комит — точка сохранения вашего кода. И стоит называть её понятно.
Ваш коллега хотел бы знать, что изменилось в коде в этом месте без необходимости смотреть код.
Уважайте ваших коллег. Не стоит называть комит "Опечатка", "Правка опечатки", "Еще один косяк" и т.п.,
...
Именования комитов разделяются на две сущности:

* название;
* описание (не обязательно).

Название — одно предложение, которое начинается с глагола и сообщает, что будет изменено в коде,
если принять комит. Предпочтительный язык — английский.

![скриншот примера комита в webstorm](https://monosnap.com/file/yxI54IFtQAVoppGm1tdXNKHKVJqEGf.png)
...
fixes #7
```
Где 7 — номер задачи https://github.com/freecodecamp-spb/web-dev-cources/issues/7, слово `fixes` означает, что комит решает задачу.
Без слова `fixes` описание делается в случае, если комит просто имеет отношение к задаче.

### Пример комита в webstorm
![Скриншот комита этого рукодства](https://monosnap.com/file/nSte0kZTU6xPpvXsOK8TDKCkWm2b4F.png)

Пример плохого названия комита:
