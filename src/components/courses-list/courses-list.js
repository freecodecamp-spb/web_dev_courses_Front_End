...
const CoursesList = (props) => {
    ...
    if (!props.courses || props.courses.length === 0) {
        ...
        coursesItems = props.courses.map(item => {
            return ( <
                ...
                onClick = {
                    () => props.deleteCourse(item) } > Delete < /button> <
    ...
    return ( <
        ...
        Вы на странице { props.start }
        из { Math.ceil(itemsCount / paginationCount) }
        страниц <
        /div>

        <
        Pagination count = { Math.min(Math.floor(itemsCount / paginationCount), paginationCount) }
        ...
        to = "/courses/new" > Добавить < /Link> <
        /div> <
        /div>


        <
        ul className = "list" > { coursesItems } <
        /ul> <
        /div>
    );
};
